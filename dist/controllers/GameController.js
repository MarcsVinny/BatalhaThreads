...
import { DATA_PROFESSORES } from '../models/Types.js';
...
function iniciarKernel() {
    ...
    DATA_PROFESSORES.forEach((prof, index) => {
        setTimeout(() => {
            ...
            const payload = { tipo: 'START', eu: prof, oponentes: oponentes };
...
function gerenciarMensagem(dados) {
    if (!isRunning)
        return;
    ...
    if (dados.stamina !== undefined) {
        ...
        if (nome)
            UI.atualizarStamina(nome, dados.stamina);
    }
...
function processarAtaque(atacante, alvoNome, dano, critico) {
    ...
    if (critico)
        msg = `💥 <b>CRÍTICO!</b> ${atacante} espancou ${alvoNome}!`;
    UI.log(msg, critico ? 'log-crit' : '');
    if (alvo.hp <= 0)
        encerrarThread(alvo, atacante);
}
function encerrarThread(prof, assassino) {
    prof.hp = 0;
...
function verificarVitoria() {
    ...
    if (vivos.length === 1) {
        ...
        if (workersAtivos[campeao.nome])
            workersAtivos[campeao.nome].terminate();
